 ...
 * @version $Revision$
 */
public class RunExportCallback extends CallbackBase {
    protected static final String NO_EXPORT_SELECTED_MESSAGE = "no.export.selected.message";

    public RunExportCallback(IDhtmlxManager manager) {
        super(CallbackConstants.RunExportCallback, manager);
    }
    ...
    public String getScriptingBody() throws DhtmlxException {
        ...
        js.ifnotc(selectedRow).thenc(new CodeBlock() {
            ...
            public void code(StringBuilder buf) throws DhtmlxException {
                ...
                // Error message
                js.alert(new JSString(localization.getMessage(NO_EXPORT_SELECTED_MESSAGE)));
            }
            ...
            public void code(StringBuilder buf) throws DhtmlxException {
                ...
                Command command = new Command(CallbackConstants.RunExportCallback);
                command.setControl(getManager());
                command.param(ParameterConstants.PARAM_ROWID, selectedRow);
                //				command.param(ParameterConstants.PARAM_EXPORTDATA, table.serialize());
                command.param(ParameterConstants.PARAM_EXPORTDATA, JSString.byRef(getManager().getControlName() + ".serializeSelectedRow()"));
