 ...
 * @version $Revision$
 */
public class UndoValidateSbgDeliveryCallback extends SimpleDeliveryButtonCallback {

    private static final String CONFIRM_UNDO_VALIDATE_MESSAGE = "confirm.undo.validate.person.message";


    private final String _stateColumnId;

    public UndoValidateSbgDeliveryCallback(IDhtmlxManager manager, String stateColumnId) {
    ...
    public String getScriptingBody() throws DhtmlxException {
        ...
        js.ifnotc(selectedRow).thenc(new CodeBlock() {
            ...
            public void code(StringBuilder buf) throws DhtmlxException {
                ...
                js.define(confirmMessage, new JSString(localization.getMessage(CONFIRM_UNDO_VALIDATE_MESSAGE)));
                js.append("var state=" + getManager().getControlName() + ".cells(selectedRow," + stateColumnIndex + ").getValue();");
                js.append("if(state==" + CodegroupUtility.MEB_DATASTATUS_DELIVERED + "){");
