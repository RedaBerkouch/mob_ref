DECLARE
    ...
    db_link_host VARCHAR2(100) := 'ABURACT_CM.BFI.ADMIN.CH'; -- ABURACT_CM | PBURACT_CM
    db_link_user VARCHAR2(100) := 'U_SCHOOL_RO';
    db_link_password VARCHAR2(100) := 'password'; -- Change
...
BEGIN
    -- Suppression du database link, s'il existe déjà
    BEGIN
    ...
    -- Construction dynamique de la vue V_SCHUL à partir de la vue V_SCHOOL sur le database link
    ...
    EXECUTE IMMEDIATE 'GRANT SELECT ON "M_MEB"."V_SCHUL" TO "U_MEB"';
    EXECUTE IMMEDIATE 'GRANT SELECT ON "M_MEB"."V_SCHUL" TO "BFS_ADMIN_ROLE"';
END;
/
