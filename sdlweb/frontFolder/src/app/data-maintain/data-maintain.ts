...
import {
	disableDeleteButtonForQualifications, disableUndoValidateButtonForQualifications, disableValidateButtonForQualifications,
...
import {
	disableDeleteButtonForLearners,
	disableUndoValidateButtonForLearners,
	disableValidateButtonForLearners,
	enrichDefaultValuesForLearners,
	enrichEditableColumnsForLearners
} from "../services/maintains/learner.utils";
import {LearnerService} from "../services/maintains/learner";
...
export class DataMaintain implements OnInit {
	...
	protected readonly learnerService = inject(LearnerService);
	...
	private readonly schoolTypeOptions = this.codeGroupStore.optionsFor('SCHOOL_DEP_TYPE', (item) => `${item.value} (${item.key})`);
	private readonly nationalityOptions = this.codeGroupStore.optionsFor('NATIONALITY', (item) => `${item.value} (${item.key})`);
	private readonly languageOptions = this.codeGroupStore.optionsFor('LANGUAGE', (item) => `${item.value} (${item.key})`);
	private readonly educationTypeOptions = this.codeGroupStore.optionsFor('EDUCATION_TYPE', (item) => `${item.value} (${item.key})`);
	private readonly planStatusOptions = this.codeGroupStore.optionsFor('TEACH_PLAN_STATUS', (item) => `${item.value} (${item.key})`);
	private readonly profMaturaOptions = this.codeGroupStore.optionsFor('PROF_MATURA', (item) => `${item.value} (${item.key})`);
	private readonly prevSchoolTypeOptions = this.codeGroupStore.optionsFor('SCHOOL_TYPE', (item) => `${item.value} (${item.key})`);
	...
	// Signals pour contrôler le clear selection du datahandler (learners)
	clearLearnersSelection = signal(0);
	...
	// Signals pour indiquer si la table des eleves contient des nouvelles eleves où à supprimmer
	containsNewLearner = signal(false);
	containsDeletedLearner = signal(false);
	containsUpdatedLearner = signal(false);
	containsModificationForLearners = computed(
		() => this.containsNewLearner() || this.containsDeletedLearner() || this.containsUpdatedLearner()
	);
	...
	private readonly optionsRegistry = {
		...
		nationalityOptions: this.nationalityOptions,
		languageOptions: this.languageOptions,
		educationTypeOptions: this.educationTypeOptions,
		planStatusOptions: this.planStatusOptions,
		profMaturaOptions: this.profMaturaOptions,
		prevSchoolTypeOptions: this.prevSchoolTypeOptions,
	} as const;
	...
	readonly qualificationsConfig = computed(() => {
		...
		const disableHandlers: Record<string, () => boolean> = {
			...
			switch_master: () => isMaster,
			prevalidate: () =>
				disableValidateButtonForQualifications(
					isMaster,
					this.containsModificationForPersons(),
			...
			undo_validate: () =>
				disableUndoValidateButtonForQualifications(
					isMaster,
					this.containsModificationForPersons(),
					this.maintainStateService.selectedQualifications(),
					roles,
	...
	// Configuration dynamique pour la table des eleves
	readonly learnersConfig = computed(() => {
		const baseConfig = tableConfigs['learners'] as DatahandlerConfig;

		const roles = this.userService.roles();
		const isMaster = this.maintainStateService.isLearnerMaster();

		const baseColumns = this.cloneColumns(baseConfig.columns ?? []);
		const baseActions = this.cloneActions(baseConfig.actions ?? []);

		const enrichedColumnsWithOptions = this.enrichColumnsWithOptions(baseColumns);
		const editable = enrichEditableColumnsForLearners(enrichedColumnsWithOptions, roles);
		...
		const enrichedColumns = enrichDefaultValuesForLearners(locked);
		...
		const disableHandlers: Record<string, () => boolean> = {
			...
			prevalidate: () =>
				disableValidateButtonForLearners(
					isMaster,
					this.containsModificationForLearners(),
					this.maintainStateService.selectedLearners(),
					roles,
				),
			undo_validate: () =>
				disableUndoValidateButtonForLearners(
			...
			save: () => !this.containsModificationForLearners(),
			block: () => !this.containsModificationForLearners(),
			add: () => false,
			delete: () =>
				disableDeleteButtonForLearners(this.maintainStateService.selectedLearners(), roles),
		};

		const actionHandlers: Record<string, () => void> = {
			export: () => this.learnerService.export(),
			switch_master: () => this.maintainStateService.setCurrentMaster('learner'),
			prevalidate: () =>
				this.dialogService.confirmAndDo('confirm.validate.message', () =>
					this.learnerService.prevalidate(),
				),
			undo_validate: () =>
				this.dialogService.confirmAndDo('confirm.undo.validate.message', () =>
					this.learnerService.undoValidate(),
				),
			save: () => console.log('on save'),
			block: () => console.log('on block'),
			add: () => console.log('on add'),
	...
	constructor() {
		...
		effect(
			() => {
				...
				const loadingLearners = this.maintainStateService.isLoadingLearners();
				...
				untracked(() => {
					...
					if (loadingLearners) this.clearLearnersSelection.set(Date.now());
				});
			});
	...
	filterChange(filters: {
		...
		this.learnerService.setParams(filters.versionFilter, canton, webFilters, filters.contextFilters);
	}
	...
	selectPersons(persons: any): void {
		if (Array.isArray(persons)) {
			...
			this.personService.selectPersons([]);
		}
	}
	...
	/** Gère la sélection de learners depuis la table */
	selectLearners(learners: any): void {
		if (Array.isArray(learners)) {
			this.learnerService.selectLearners(learners);
		} else {
			this.learnerService.selectLearners([]);
		}
	}
	...
	handleTableEventforLearners(event: TableEvent): void {
		this.handleTableEvent<Learner>(
			event,
			this.learnerService,
			this.containsNewLearner,
			this.containsDeletedLearner,
			this.containsUpdatedLearner,
			this.cleanItems,
			'learners'
		);
	}
