...
export class CantonService {
	private readonly baseUrl: string = '/sdlweb/api/initialisations/';
	...
	readonly webFilters: Signal<WebFilter[]> = toSignal(
		this.http.get<WebFilter[]>(`${this.cantonsUrl}/predefined_filters`).pipe(
			filter(value => !!value)
		),
		{initialValue: []}
	);
	...
	readonly lastFilters: Signal<LastFilters | undefined> = toSignal(
		this.http.get<LastFilters>(`${this.cantonsUrl}/lastFilters`).pipe(
			filter(value => !!value)
		),
	...
	exportCSV() {
		...
		this.http.post<Blob>(`${this.cantonsUrl}/export_csv`, body, {
			responseType: 'blob' as 'json',
			observe: 'response'
		}).pipe(
			finalize(() => console.log('Export completed'))
	...
	showPlausiReport() {
		...
		this.http.get(`${this.cantonsUrl}/show_last_plausi_report/${cantonId}`, {
			responseType: 'blob',
			observe: 'response'
		}).pipe(
	...
	create(cantons: Canton[]):void {
		...
		const requests$ = cantons.map(canton =>
			this.http.put<Canton>(`${this.cantonsUrl}`, canton)
				.pipe(
					catchError(error => {
	...
	update(cantons: Canton[]): void {
		...
		const requests$ = cantons.map(value =>
			this.http.post<Canton>(`${this.cantonsUrl}`, value)
				.pipe(
					catchError(error => {
						let errorMessage = 'Erreur lors de la sauvegarde d\'un canton:';
						this.opError.set(error?.error || errorMessage);
	...
	delete(cantons: Canton[]): void {
		...
		const requests$ = cantons.map(canton =>
			this.http.delete<Canton>(`${this.cantonsUrl}/${canton.cantonId}`)
				.pipe(
					catchError(error => {
	...
	getMissingCantons(): void {
		...
		this.http.get<number[]>(`${this.cantonsUrl}/missing_cantons/`, {params})
			.pipe(
				catchError(err => {
	...
	public getPlausiErrors(cantonId: number): Observable<PlausiError[]> {
		...
		return this.http.get<PlausiError[]>(`${this.cantonsUrl}/${cantonId}/plausi_errors`)
			.pipe(
				map( (plausiErrors) => {
	...
	private doLoadCantons(cantonParams: CantonParams): void {
		...
		this.http.post<Canton[]>(`${this.cantonsUrl}/search`, body)
			.pipe(
				catchError(err => {
	...
	private executeCantonAction(action: string, errorMessage: string): void {
		...
		this.executeCantonAction$(action, errorMessage)
			.subscribe();
	}
	...
	private executeCantonAction$(action: string, errorMessage: string) {
		...
		return this.http.post<Canton>(`${this.cantonsUrl}/${action}/${cantonId}`, {})
			.pipe(
				tap(canton => {
					if (canton) {
