import {CantonIntervention, SDL_CANTONINTERVENTIONTYPE} from "../../model/CantonIntervention";
import {DatahandlerColumn} from "../../shared/datahandler/datahandler";

export type ActionTypeForCantonIntervention =
	| 'download_file'      // BUTTON_DOWNLOAD_FILE - 7
...
export function isActionDisabledForCantonIntervention(
	...
	switch (actionType) {
		case "delete":
			return !hasSelected || (isDL && type < SDL_CANTONINTERVENTIONTYPE.MANUAL);
		...
		case "show_plausi_report":
			return !hasSelected || type != SDL_CANTONINTERVENTIONTYPE.CREATE_PLAUSIREPORT;

		case "download_file":
			return !hasSelected || type != SDL_CANTONINTERVENTIONTYPE.UPLOAD;
...
export function enrichDefaultValuesForCantonIntervention(columns: DatahandlerColumn[]): DatahandlerColumn[] {
	const defaultValues: Record<string, any> = {
		...
		type: SDL_CANTONINTERVENTIONTYPE.MANUAL
	};
