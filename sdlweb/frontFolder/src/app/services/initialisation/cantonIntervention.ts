...
export class CantonInterventionService {
	...
	/* ************* */
	downloadFile() {
		const currentCantonIntervention = this.currentCantonIntervention();

		if (!currentCantonIntervention) {
			return;
		}

		let interventionId = currentCantonIntervention.interventionId;

		this.http.get(`${this.cantonInterventionsUrl}/download_file/${interventionId}`, {
			responseType: 'blob',
			observe: 'response'
		}).pipe(
			finalize(() => console.log('Downloading file completed'))
		).subscribe({
			next: (response: HttpResponse<Blob>) => {
				openCsvFile(response, 'file.xlsx');
			},
			error: (error) => {
