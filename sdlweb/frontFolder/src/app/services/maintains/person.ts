...
export class PersonService {
	private readonly baseUrl = '/sdlweb/api/persons';
	...
	delete(persons: Person[]): void { // TODO gérer quand c'est pas Master
		...
		const requests$ = persons.map(person =>
			this.http.delete<void>(`${this.baseUrl}/${person.schoolId}/${registerWithoutPlausi}`).pipe(
				catchError(error => {
					const errorMessage = `Erreur lors de la suppression de la personne ${person.schoolId}:`;
	...
	private executePersonAction$(action: string, errorMessage: string): Observable<Person | null> {
		...
		const body = {
			personIds: selectedPersons.map(person => person.schoolId)
		};
