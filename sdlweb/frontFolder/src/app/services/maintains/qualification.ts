...
export class QualificationService {
	private readonly baseUrl = '/sdlweb/api/qualifications';
	...
	private learnerSelectionSubject = new Subject<number[]>();
	private selectedLearnerIds = signal<number[]>([]);
	...
	private readonly personIdsToLoad = computed(() => {
		...
		const isPersonMaster = this.maintainState.isPersonMaster();

		if (isPersonMaster && !isLoading) {
	...
	private readonly learnerIdsToLoad = computed(() => {
		const isLoading = this.maintainState.isLoadingPersons();
		const learners = this.maintainState.selectedLearners();
		const isLearnerMaster = this.maintainState.isLearnerMaster();

		if (isLearnerMaster && !isLoading) {
			return learners.length > 0
				? learners
					.filter(p => p?.learnerId)
					.map(p => p.learnerId)
				: [];
		}
		return null;
	...
	constructor() {
		this.destroyRef.onDestroy(() => {
			...
			this.learnerSelectionSubject.complete();
		});
		...
		// 4. Effect pour non-Master
		effect(() => {
			const learnerIds = this.learnerIdsToLoad();

			if (learnerIds !== null && learnerIds.length > 0) {
				untracked(() => {
					this.qualifications.set([]);
					if (learnerIds.length !== 0) {
						this.learnerSelectionSubject.next(learnerIds);
					}
				});
			}
		});

		this.learnerSelectionSubject
			.pipe(
				debounceTime(500),
				distinctUntilChanged((a, b) =>
					a.length === b.length && a.every((v, i) => v === b[i])
			...
			.subscribe(learnerIds => {
				this.selectedLearnerIds.set(learnerIds);
				this.loadQualificationsByLeaners(learnerIds);
			});
	...
	/**
	 * Charge les qualifications pour les learner sélectionnées (isMaster = false)
	 */
	private loadQualificationsByLeaners(learnerIds: number[]): void {
		if (learnerIds.length === 0) {
			this.qualifications.set([]);
			return;
		}

		console.log(`Chargement des qualifications pour ${learnerIds.length} learner(s)...`);

		// Créer les query params
		let params = new HttpParams();
		learnerIds.forEach(id => {
			params = params.append('learnerIds', id.toString());
		});
	...
	/**
	 * Recharge les Qualifications par learner (non-master mode) - version Observable
	 */
	private reloadQualificationsByLearners(learnerIds: number[]): Observable<Qualification[]> {
		if (learnerIds.length === 0) {
			return of([]);
		}
	...
	create(qualifications: Qualification[]): void {
		...
		const body = qualifications.map(value => ({
			...
			qualification: value,
			personId: (selectedPersons.length > 0) ? selectedPersons[0].schoolId : undefined
		}));
	...
	delete(qualifications: Qualification[]): void {
		...
		const requests$ = qualifications.map(qualification =>
			this.http.delete<void>(`${this.baseUrl}/${qualification.classId}/${registerWithoutPlausi}`)
				.pipe(
					catchError(error => {
						let errorMessage = `Erreur lors de la suppression de la qualification ${qualification.schoolId}:`;
