import {Learner, MEB_DATASTATUS, Qualification} from "../../model/Maintain";
...
export function calculateLearnerCondition(learner: Learner): string {
	const deliveryStatus = learner.deliveryStatus;
	const plausiStatus = learner.plausiStatus;
...
export function enrichEditableColumnsForLearners(columns: DatahandlerColumn[], roles: string[]): DatahandlerColumn[] {
	...
	return columns.map(col => {
		...
		if (isEditableColumn) {
			clonedCol.editableOnCondition = (row: Learner, rowIndex: number) => {
				// 1. Nouvelle ligne
 ...
 */
export function disableValidateButtonForLearners(isMaster: boolean, containsModification: boolean, selectedLearners: Learner[], roles: string[]): boolean {
	if (!isMaster || containsModification || selectedLearners.length === 0) {
		return true;
	}

	// Trouver minState et maxState
	const states = selectedLearners.map(p => p.deliveryStatus);
 ...
 */
export function disableUndoValidateButtonForLearners(isMaster: boolean, containsModification: boolean, selectedLearners: Learner[], roles: string[]): boolean {
	if (!isMaster || containsModification || selectedLearners.length === 0) {
		return true;
	}
 ...
 */
export function disableDeleteButtonForLearners(selected: Learner[] | Qualification[], roles: string[]): boolean {
...
export function enrichDefaultValuesForLearners(columns: DatahandlerColumn[]): DatahandlerColumn[] {
