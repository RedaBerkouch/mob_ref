...
export class DeliveryService {
	private readonly baseUrl = '/sdlweb/api/deliveries';
	...
	private formatDelivery(delivery: Delivery): Delivery {
		return {
			...
			nrPlausiClass: this.translateLoadingState(delivery.nrPlausiClass),
			nrPlausiLearner: this.translateLoadingState(delivery.nrPlausiLearner),
			nrPlausiSchool: this.translateLoadingState(delivery.nrPlausiSchool)
		};
	}
	...
	private needsPolling(delivery: Delivery): boolean {
		return this.LOADING_STATES.includes(delivery.nrPlausiClass as any)
			|| this.LOADING_STATES.includes(delivery.nrPlausiLearner as any)
			|| this.LOADING_STATES.includes(delivery.nrPlausiSchool as any);
	}

	private needsRefreshPolling(delivery: Delivery): boolean {
		return delivery.deliveryStatus === MEB_DELIVERYSTATUS.IMPORTED || (delivery as any).creatingReport;
	}
