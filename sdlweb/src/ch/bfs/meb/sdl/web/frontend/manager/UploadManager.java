...
import ch.bfs.meb.sdl.web.service.UploadFileService;
...
import ch.bfs.meb.sdl.web.ws.sdlupload.UploadResult;
...
public class UploadManager extends DhtmlxManagerBase {
    ...
    @Autowired
    CantonInterventionTableManager _cantonInterventionTableManager;
    ...
    protected DojoIframeIOHTML doDelivery(MultipartFile file, boolean wizard) {
        ...
        try {
            if (upperFileName.endsWith(".XML") || upperFileName.endsWith(".CSV")) {
                // 🔹 Conversion UTF-8 comme dans SBG
                ...
                // 🔹 Traitement ZIP (conversion interne UTF-8)
            ...
            return new DojoIframeIOHTML(
                    getLocalizationManager().getMessage("upload.invalidEncoding.message",
        ...
        boolean unconfiguredSchoolsAndOneCantonPerDeliveryError =
                state.getUnconfiguredSchoolIds().size() > 0 &&
        ...
        if (!unconfiguredSchoolsAndOneCantonPerDeliveryError && !StringUtils.isEmpty(state.getMessage())) {
            ...
            return new DojoIframeIOHTML(messageText.toString());
        } else {
            return new DojoIframeIOHTML();
        }
    }
    ...
    public DojoIframeIOHTML delivery(MultipartFile file) throws IOException {
