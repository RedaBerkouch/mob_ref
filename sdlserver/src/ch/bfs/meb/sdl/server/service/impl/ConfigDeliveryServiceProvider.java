...
public class ConfigDeliveryServiceProvider implements IConfigDeliveryServiceProvider {
    ...
    protected void initConfigDeliveryUsers(SdlConfigDelivery delivery) {
        boolean first = true;
        String dlUsers = "";
        int anz = 0;
        for (String user : delivery.getDl_users().split(USER_TEXT_DELIMITER)) {
            if (_idmService.isUserInRole(user, SecurityConstants.ROLE_SDL_DL, SecurityConstants.SDL_ROLE_HIERARCHY)) {
                if (!first) {
        ...
        for (String user : delivery.getRo_users().split(USER_TEXT_DELIMITER)) {
            if (_idmService.isUserInRole(user, SecurityConstants.ROLE_SDL_RO, SecurityConstants.SDL_ROLE_HIERARCHY)) {
                if (!first) {
    ...
    public void copyConfigDeliveries(Long newVersion, Long version, Long canton) {
        List<SdlConfigDelivery> deliveries = _configDeliveryRepository.getConfigDeliveriesByVersionAndCanton(version, canton);
        for (SdlConfigDelivery delivery : deliveries) {
            SdlConfigDelivery newDelivery = new SdlConfigDelivery(delivery);
    ...
    public ConfigDelivery insertConfigDelivery(ConfigDelivery configDelivery) {
        return _configDeliveryRepository.insertConfigDelivery(new SdlConfigDelivery(configDelivery));
    }

    @Override
    public ConfigDelivery updateConfigDelivery(ConfigDelivery configDelivery) {
        SdlConfigDelivery delivery = new SdlConfigDelivery(configDelivery);
        SdlConfigDelivery origDelivery = _configDeliveryRepository.getConfigDeliveryById(delivery.getDeliveryId());
        delivery.setBurSchools(origDelivery.getBurSchools());
        if (!delivery.getDeliveryCode().equals(origDelivery.getDeliveryCode())) {
            // update changed configDeliveryCode in delivery, school, class and learner
    ...
    public void deleteConfigDelivery(ConfigDelivery configDelivery) {
        _configDeliveryRepository.deleteConfigDelivery(new SdlConfigDelivery(configDelivery));
    }
}
