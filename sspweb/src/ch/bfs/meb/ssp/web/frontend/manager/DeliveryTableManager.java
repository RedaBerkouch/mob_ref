...
import ch.bfs.meb.ssp.web.dhtmlx.callback.RefreshDeliveryButtonsCallback;
import ch.bfs.meb.ssp.web.frontend.resultmapper.DeliveryListTableResultMapper;
import ch.bfs.meb.ssp.web.frontend.resultmapper.DeliveryTableResultMapper;
...
public class DeliveryTableManager extends FilteredTableManagerBase implements PlausiErrorColumn.IPlausiErrorDataUpdate {
    ...
    public static final String COLUMN_PLAUSIACTIVITY_ID = "nrPlausiActivity";
    public static final String COLUMN_PLAUSIACTIVITY_NAME_KEY = "deliveryTable.column.plausiactivity.name";
    ...
    public void create() throws DhtmlxException {
        ...
        if (user.getLastFilterVersion() == null) {
            setFilterVersion(FilterContextUtility.getActVersion(_filterService, CodegroupUtility.SSP_OBJECTTYPE_CONFIGURATION));
        ...
        addColumn(new ReadOnlyColumn(COLUMN_PLAUSIACTIVITY_ID, COLUMN_PLAUSIACTIVITY_NAME_KEY, false, true, getLocalizationManager(), 20));
        ...
        onColumnSortCallback.addIgnoreColumn(getColumnIndexById(COLUMN_PLAUSIACTIVITY_ID));
        ...
        registerCallback(new ShowReplaceDataCallback(this, COLUMN_PLAUSIPERSON_ID, COLUMN_PLAUSIACTIVITY_ID));
        ...
        registerCallback(new ValidateDeliveryCallback(this, CodegroupUtility.MEB_APPLICATION_SSP));
        ...
        registerCallback(new DeleteDeliveryCallback(this, COLUMN_STATUS_ID, CodegroupUtility.MEB_APPLICATION_SSP));
    ...
    protected Long getDeliveryId(String selected) {
        return ((SspDelivery) getRowData(selected)).getDeliveryId();
    }
     ...
     *         rows
     */
    public SspDeliveryListResult getRows(int start, int buffer) {
     ...
     */
    public SspDeliveryListResult getRows(ParameterList params) {
     ...
     */
    @Override
    protected List<SspDelivery> getExportRows(ParameterList params) {
    ...
    public DhtmlxTableXML init(ParameterList params) throws DhtmlxException {
        if (params.getRowsLoaded() == 0) {
            ...
            SspDeliveryListResult result = getRows(params);
    ...
    public IHttpResult filter(ParameterList params) throws DhtmlxException {
        ...
        SspDeliveryListResult result = getRows(rowsloaded, bufferlen);
        clearPlausiErrorData(result.getDeliveries());
        resultSize = result.getMaxNrOfDeliveries();
    ...
    public DhtmlxTableDataXML amend(ParameterList params) throws DhtmlxException {
        ...
        SspDeliveryResult result = _deliveryService.amendDelivery(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML replace(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.replaceDelivery(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML confirm(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.confirmDelivery(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML cancel(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.cancelDelivery(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML validate(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.validateDelivery(getDeliveryId(selected), false);
    ...
    public DhtmlxTableDataXML undoValidate(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.validateDelivery(getDeliveryId(selected), true);
    ...
    public FileHttpResult showLastPlausireport(ParameterList params) throws DhtmlxException {
        ...
        try {
            ...
            for (int i = 0; i < Math.min(16, returnValue.length); i++) {
                System.out.printf("%02X ", returnValue[i]);
            }
    ...
    public DhtmlxTableDataXML createPlausireport(ParameterList params) throws DhtmlxException {
        ...
        SspDeliveryResult result = _deliveryService.createPlausireport(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML update(ParameterList params) throws DhtmlxException, OgnlException {
        ...
        SspDelivery delivery = (SspDelivery) merge(params);
        PlausiErrorColumn.setPlausiErrorData(delivery.getDeliveryId(), params, this);

        SspDeliveryResult result = _deliveryService.updateDelivery(delivery, _loadedPlausiErrors.get(delivery.getDeliveryId()));
    ...
    public DhtmlxTableDataXML delete(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDeliveryResult result = _deliveryService.deleteDelivery(getDeliveryId(selected));
    ...
    public DhtmlxTableDataXML refreshStatus(ParameterList params) throws DhtmlxException {
        String selected = params.getRowId();
        SspDelivery beforeRefresh = (SspDelivery) getRowData(selected);
        ...
        SspDeliveryResult result = _deliveryService.refreshStatus(beforeRefresh);
    ...
    protected void clearPlausiErrorData(List<SspDelivery> deliveries) {
    ...
    public class ReplaceDeliveryData implements Runnable {
        ...
        final SspDelivery _delivery;

        public ReplaceDeliveryData(SspDelivery delivery, SecurityContext securityContext) {
            _delivery = delivery;
        ...
        public void run() {
            ...
            try {
                SspDeliveryResult result = _deliveryService.getDeliveryById(_delivery.getDeliveryId());
                if (result.getState() != ResultBase.OK) {
                    _delivery.setNrPlausiActivity("");
    ...
    public synchronized IHttpResult replaceData(ParameterList params) throws DhtmlxException {
        String sid = params.getRowId();
        SspDelivery delivery = (SspDelivery) getRowData(sid);
        if (delivery.getNrPlausiPerson().equals(MebUtils.getDeliveryToBeLoadedMessage())) {
            ...
            return toReplaceDataResponse(sid, _localizationManager.getMessage(delivery.getNrPlausiPerson(), null, delivery.getNrPlausiPerson()),
                    delivery.getNrPlausiActivity(), "");
        }
    }

    private boolean isReadOnly(SspDelivery delivery) {
        MebUser user = (MebUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
    ...
    public Map<String, String> getRowUserData(Object row) {
        HashMap<String, String> userData = new HashMap<String, String>();
        SspDelivery delivery = (SspDelivery) row;
        String readOnlyStr;

        // COLUMN_CODE, COLUMN_CANTON, COLUMN_VERSION, COLUMN_STATUS, COLUMN_PLAUSISTATUS
        // COLUMN_DELIVERYDATE, COLUMN_DELIVERYUSER, COLUMN_PLAUSIPERSON, COLUMN_PLAUSIACTIVITY
    ...
    protected String getRowStyleClass(Object row) {
        SspDelivery sspDelivery = (SspDelivery) row;

        if (sspDelivery.getDeliveryStatus() == null) {
            return ROW_NOT_VALID_STYLE;
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_FINALIZED) {
            return ROW_FINALIZED_STYLE;
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_VALIDATED) {
            return ROW_VALIDATED_STYLE;
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_PREVALIDATED) {
            return ROW_PREVALIDATED_STYLE;
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_DELIVERED) {
            if (sspDelivery.getPlausiStatus() >= CodegroupUtility.MEB_PLAUSISTATUS_VALID) {
                return ROW_VALID_STYLE;
            } else {
                return ROW_NOT_VALID_STYLE;
            }
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_IMPORTED) {
            return ROW_DELIVERY_IMPORTED_STYLE;
        } else if (sspDelivery.getDeliveryStatus() == CodegroupUtility.MEB_DELIVERYSTATUS_INITIALIZED) {
