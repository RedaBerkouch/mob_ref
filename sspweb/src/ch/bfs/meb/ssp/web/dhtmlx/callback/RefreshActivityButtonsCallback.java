/*
  ...
  $Id: RefreshActivityButtonsCallback.java 305 2009-12-03 10:25:28Z jfu $

 */
package ch.bfs.meb.ssp.web.dhtmlx.callback;
...
/**
 * Refresh callback to enable/disable the activity table buttons
 * 
 * @author $Author: jfu $
 * @version $Revision: 305 $
 */
public class RefreshActivityButtonsCallback extends CallbackBase {
    ...
    public RefreshActivityButtonsCallback(IDhtmlxManager manager, IDhtmlxControl parentTable, String stateColumnId, String parentStateColumnId) {
    ...
    public String getScriptingBody() {
        ...
        if (_user.isInRole(SecurityConstants.ROLE_SSP_DL)) {
            if (_user.isInRole(SecurityConstants.ROLE_SSP_DV)) {
                if (_user.isInRole(SecurityConstants.ROLE_SSP_EV)) {
                    // EV or EA
        ...
        } else if (_user.isInRole(SecurityConstants.ROLE_SSP_DV)) {
            // DV
            js.append("disableInsert=state>=" + CodegroupUtility.MEB_DELIVERYSTATUS_VALIDATED + ";");
        } else if (_user.isInRole(SecurityConstants.ROLE_SSP_DL)) {
            // DL
