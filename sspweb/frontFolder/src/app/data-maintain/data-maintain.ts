...
export class DataMaintain implements OnInit {
	...
	private readonly nationalityStatusOptions = this.codeGroupStore.optionsFor('NATIONALITY', (item) => `${item.value} (${item.key})`);
	private readonly persCategoryOptions = this.codeGroupStore.optionsFor('PERS_CATEGORY', (item) => `${item.value} (${item.key})`);
	private readonly contractTypeOptions = this.codeGroupStore.optionsFor('TYPE_CONTRACT', (item) => `${item.value} (${item.key})`);
	private readonly qualificationOptions = this.codeGroupStore.optionsFor('QUALIFICATION', (item) => `${item.value} (${item.key})`);
	...
	private readonly optionsRegistry = {
		...
		nationalityStatusOptions: this.nationalityStatusOptions,
		persCategoryOptions: this.persCategoryOptions,
