...
import {catchError, filter, finalize, interval, of, retry, switchMap, takeWhile, tap} from 'rxjs';
...
export class DeliveryService {
	private readonly baseUrl = '/sspweb/api/deliveries';
	...
	private formatDelivery(delivery: Delivery): Delivery {
		return {
			...
			nrPlausiActivity: this.translateLoadingState(delivery.nrPlausiActivity)
		};
	}
	...
	private needsPolling(delivery: Delivery): boolean {
		return this.LOADING_STATES.includes(delivery.nrPlausiPerson as any)
			|| this.LOADING_STATES.includes(delivery.nrPlausiActivity as any);
	}
