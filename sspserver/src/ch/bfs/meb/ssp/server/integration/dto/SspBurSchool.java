...
/** Entity for the SSP {@link BurSchool} data table. */
@Data
@Entity
@Table(name = "SCHOOLS")
...
public class SspBurSchool extends BurSchool {
    @Column
    @ManyToMany(targetEntity = SspConfigDelivery.class, fetch = FetchType.EAGER, cascade = { CascadeType.PERSIST, CascadeType.MERGE })
    @JoinTable(name = "SSP_SCHOOLS_CONFIGDELIVERIES", joinColumns = @JoinColumn(name = "schoolId"), inverseJoinColumns = @JoinColumn(name = "deliveryId"))
    protected List<SspConfigDelivery> configDeliveries;

    public SspBurSchool() {}

    /** Copy constructor */
    public SspBurSchool(BurSchool dtoBurSchool) {
