...
import {DatahandlerConfig} from "../shared/datahandler/datahandler";
...
import tableConfigs from '../../assets/tables-datatracking-config.json';
import {Delivery, Export, Intervention, MEB_INTERVENTIONTYPE, WebFilter} from "../model/Delivery";
...
@Component({
	...
	standalone: false,
	templateUrl: './data-tracking.html',
	styleUrl: './data-tracking.scss',
})
export class DataTracking implements OnInit {
	...
	private deliveryStatusOptions = this.mapCodeGroupToOptions('SBG_DELIVERYSTATUS');
	private plausiStatusOptions = this.mapCodeGroupToOptions('SBG_PLAUSISTATUS');
	private interventionStatusOptions = this.mapCodeGroupToOptions('SBG_ACTIONTYPE');
	...
	livraisonsConfig = computed(() => {
		...
		const deleteMessage = 'confirm.delete.delivery.message';

		const undoValidateMessage = 'confirm.undo.validate.message';
		...
		config.columns
			...
			.forEach((col) => {
				col.editableOnCondition = (row: Intervention, rowIndex) => {
					return row.type === MEB_INTERVENTIONTYPE.UPLOAD;
				};
			});
	...
	private loadAllCodeGroups(): void {
		...
		this.codeGroupStore.loadValues('SBG_DELIVERYSTATUS');
		this.codeGroupStore.loadValues('SBG_PLAUSISTATUS');
		this.codeGroupStore.loadValues('SBG_ACTIONTYPE');
	}
