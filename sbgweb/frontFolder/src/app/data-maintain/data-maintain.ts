...
export class DataMaintain implements OnInit {
	...
	private readonly statusOptions = this.codeGroupStore.optionsFor('SBG_PERSONSTATUS');
	private readonly sbfiCodeOptions = this.codeGroupStore.optionsFor('SBG_SBFICODE');
	private readonly professionCodeOptions = this.codeGroupStore.optionsFor('PROFESSIONCODE', (item) => `${item.value} (${item.key})`);
	private readonly typeOptions = this.codeGroupStore.optionsFor('SBG_EVENTTYPE', (item) => `${item.value} (${item.key})`);
	private readonly contractTypeOptions = this.codeGroupStore.optionsFor('CONTRACTTYPE');
	private readonly cancelReasonOptions = this.codeGroupStore.optionsFor('CANCELREASON');
	private readonly examTypeOptions = this.codeGroupStore.optionsFor('EXAMTYPE');
	...
	private readonly plausiStatusOptions = this.codeGroupStore.optionsFor('SBG_PLAUSISTATUS');
	private readonly maturityLanguagesOptions = this.codeGroupStore.optionsFor('SBA_MATURITY_LANGUAGES');
	private readonly examResultOptions = this.codeGroupStore.optionsFor('EXAMRESULT', (item) => `${item.value} (${item.key})`);
	...
	private readonly optionsRegistry = {
		...
		sbfiCodeOptions: this.sbfiCodeOptions,
		professionCodeOptions: this.professionCodeOptions,
		typeOptions: this.typeOptions,
		contractTypeOptions: this.contractTypeOptions,
		cancelReasonOptions: this.cancelReasonOptions,
	...
	readonly personsConfig = computed(() => {
		...
		const locked = enrichedColumnsWithOptions.map((c) =>
