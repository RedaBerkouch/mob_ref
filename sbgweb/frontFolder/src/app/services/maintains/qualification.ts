...
export class QualificationService {
	private readonly baseUrl = '/sbgweb/api/qualifications';
	...
	create(qualifications: Qualification[]): void {
		...
		const body = qualifications.map(value => ({
			...
			qualification: value,
			personId: (selectedPersons.length > 0) ? selectedPersons[0].pid : undefined
		}));
	...
	delete(qualifications: Qualification[]): void {
		...
		const requests$ = qualifications.map(qualification =>
			this.http.delete<void>(`${this.baseUrl}/${qualification.eventid}/${registerWithoutPlausi}`)
				.pipe(
					catchError(error => {
						let errorMessage = `Erreur lors de la suppression de la qualification ${qualification.eventid}:`;
