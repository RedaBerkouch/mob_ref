...
export class AdminExportParamService {
	...
	loadAll(exportId: number): void {
		this.http.get<ParameterListResult>(`${this.baseUrl}/${exportId}/parameters`).subscribe({
			next: (result) => {
				if (result?.state === 1 && Array.isArray(result.parameters)) {
					this._parameters.set(result.parameters);
				} else {
					console.warn('Réponse inattendue paramètres export', result);
	...
	getById(exportId: number, paramId: number): Observable<WebFilterParameter | undefined> {
		return this.http
			.get<ParameterResult>(`${this.baseUrl}/${exportId}/parameters/${paramId}`)
			.pipe(map((result) => result.parameter));
	}

	create(exportId: number, param: WebFilterParameter): void {
		this.http.post<ParameterResult>(`${this.baseUrl}/${exportId}/parameters`, param).subscribe({
			next: (result) => {
				if (result?.state === 1 && result.parameter) {
					this._parameters.set([...this._parameters(), result.parameter]);
				} else {
					console.warn('Réponse inattendue création param export', result);
				}
			},
	...
	update(exportId: number, paramId: number, param: WebFilterParameter): void {
		this.http
			.put<ParameterResult>(`${this.baseUrl}/${exportId}/parameters/${paramId}`, param)
			.subscribe({
				next: (result) => {
					if (result?.state === 1 && result.parameter) {
						this._parameters.set(
						...
						console.warn('Réponse inattendue update param export', result);
					}
				},
			...
			.subscribe({
				next: (result) => {
					if (result?.state === 1) {
						...
						console.warn('Réponse inattendue suppression param export', result);
					}
				},
