...
export function calculateDeliveryCondition(delivery: Delivery): string {
	...
	if (deliveryStatus === MEB_DELIVERYSTATUS.EMPTY) {
		return DATAHANDLER_STYLE.NOT_VALID;
	}
...
function extractRoles(roles: string[]) {
	return {
		hasDL: roles.includes(Role.SBG_DL),
		hasDV: roles.includes(Role.SBG_DV),
		hasEV: roles.includes(Role.SBG_EV),
		hasEA: roles.includes(Role.SBG_EA)
	};
}
