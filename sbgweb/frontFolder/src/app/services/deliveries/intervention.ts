...
export class InterventionService {
	private readonly baseUrl = '/sbgweb/api/deliveries/interventions';
	...
	readonly interventions: Signal<Export[]> = computed(() => {
		...
		return data.map(intervention => ({
			...intervention,
			validationreport: getTranslation(intervention, 'validationreport', lang)
		}));
	});
	...
	showPlausiReportIntervention() {
		const interventionId = this.currentIntervention()?.actionid;
	...
	insertInterventions(interventions: Intervention[]): void {
		...
		const body = interventions.map(value => ({
			...
			type: value.type,
			validationreportDe: value.validationreportDe,
			validationreportFr: value.validationreportFr,
			validationreportIt: value.validationreportIt,
		} as Intervention));
	...
	deleteInterventions(interventions: Intervention[]): void {
		...
		const requests$ = interventions.map(intervention =>
			this.http.delete<void>(`${this.baseUrl}/${intervention.actionid}`)
				.pipe(
					catchError(error => {
						console.error(`Erreur lors de la suppression de l'intervention ${intervention.actionid}:`, error);
						return of(null);
					})
				)
		);
