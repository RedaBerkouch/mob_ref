...
import ch.bfs.meb.sba.server.service.xmlbeans.TableDocument.Table;
...
/**
 * Business object for handling Sba deliveries.
 * 
 * @author $Author: dzw $
...
public class DeliveryBO extends BOBase {
    ...
    protected SbaDelivery _thisDelivery = null;
     ...
     * 
     * @param sbaDelivery	database object
     */
    public DeliveryBO(SbaDelivery delivery) {
     ...
     * @param sbaDelivery	database object
     */
    public DeliveryBO(SbaDelivery delivery, boolean loadAll, IPersonRepository personRepository, IQualificationRepository qualificationRepository) {
        ...
        if (loadAll) {
            Set<SbaPerson> existingPersons = personRepository.loadWholeDelivery(_thisDelivery.getDeliveryId());
            for (SbaPerson person : existingPersons) {
                _persons.add(new PersonBO(person, true, qualificationRepository));
            }
        }

        for (SbaPlausiError plausierror : delivery.getPlausierrors()) {
    ...
    public void initialize() {
        ...
        if (_thisDelivery == null) {
            ...
            _thisDelivery.setIsLocked(SbaDelivery.DELIVERY_NOT_LOCKED);
            _firstDelivery = true;
    ...
    public SbaDelivery getThisDelivery() {
