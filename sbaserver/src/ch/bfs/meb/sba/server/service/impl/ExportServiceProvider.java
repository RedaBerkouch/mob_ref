...
import ch.bfs.meb.sba.server.service.xmlbeans.MEBResponseDocument;
import ch.bfs.meb.sba.server.service.xmlbeans.MEBResponseDocument.MEBResponse;
import ch.bfs.meb.sba.server.service.xmlbeans.MEBResponseDocument.MEBResponse.ValidationErrors;
import ch.bfs.meb.sba.server.service.xmlbeans.ResponseType;
import ch.bfs.meb.sba.server.service.xmlbeans.TableDocument;
import ch.bfs.meb.sba.server.service.xmlbeans.ValidationErrorType;
...
public class ExportServiceProvider extends ExportServiceProviderBase {
    ...
    public FileResult runXmlExport(Export export, String locale) {
        ...
        SbaDelivery delivery = deliveryRepository.getDeliveryByIdentification(canton, version, deliveryCode);
        if (delivery == null)
            // No delivery error
        ...
        if (!user.isInRole(SecurityConstants.ROLE_SBA_EV)) {
            if (user.isInRole(SecurityConstants.ROLE_SBA_DV)) {
                ...
                SbaConfigDelivery configDelivery = configDeliveryRepository.getConfigDeliveryByCodeVersionAndCanton(delivery.getConfigDeliveryCode(), version,
                        canton);
                ...
                } else if (user.isInRole(SecurityConstants.ROLE_SBA_RO)) {
        ...
        DeliveryBO deliveryBO = new DeliveryBO(delivery, true, personRepository, qualificationRepository);
    ...
    public FileResult runUsersExport(Export export, String locale) {
        ...
        // Get all sba users
        List<User> roUsers = idmService.getUsersForRole(SecurityConstants.ROLE_SBA_RO);
        List<User> dlUsers = idmService.getUsersForRole(SecurityConstants.ROLE_SBA_DL);
        List<User> dvUsers = idmService.getUsersForRole(SecurityConstants.ROLE_SBA_DV);

        // Get all data
        List<SbaDelivery> deliveries = deliveryRepository.getDeliveries(-1, -1, new SortContext(), new FilterContext(), version, -1L);
        List<SbaConfigDelivery> configDeliveries = configDeliveryRepository.getConfigDeliveriesByVersion(version);
    ...
    private List<String> getConfigDeliveryCodesForDL(List<SbaConfigDelivery> configDeliveries, ExportUser dl) {
    ...
    private List<String> getConfigDeliveryCodesForRO(List<SbaConfigDelivery> configDeliveries, ExportUser ro) {
        List<String> deliveryCodes = new ArrayList<>();
        for (SbaConfigDelivery configDelivery : configDeliveries) {
    ...
    private void setMinDeliveryStatus(ExportUser user, List<SbaDelivery> deliveries) {
        Long minStatus = null;
    ...
    public FileResult runInitStatusExport(Export export, String locale) {
        ...
        List<SbaBurSchool> sbaSchools = burSchoolRepository.getNotConfiguredSchoolsForVersion(version);
        List<BurSchool> schools = new ArrayList<>();
        schools.addAll(sbaSchools);

        // Get all sba users (DL and RO)
        HashMap<String, ExportUser> users = new HashMap<>();
        List<User> eaUsers = idmService.getUsersForRole(SecurityConstants.ROLE_SBA_EA);
        addAll(users, eaUsers, SecurityConstants.ROLE_EA);
        List<User> evUsers = idmService.getUsersForRole(SecurityConstants.ROLE_SBA_EV);
        ...
        List<SbaConfigDelivery> sbaConfigDeliveries = configDeliveryRepository.getConfigDeliveriesByVersion(version);
        List<ConfigDelivery> configDeliveries = new ArrayList<>();
        configDeliveries.addAll(sbaConfigDeliveries);
    ...
    public FileResult runXmlDeliveryPlausireportExport(Export export, String locale) {
        ...
        SbaDelivery delivery = evaluateParameters(version, canton, deliveryCode);

        // Load the business objects
        // Get all plausi data from database and build list and HashMap of
        // relevant plausis
        List<SbaPlausi> allPlausiList = plausiRepository.getPlausis();
        ...
        List<SbaPlausiError> plausiErrors = plausiErrorRepository.getPlausiErrorsForDelivery(delivery.getDeliveryId());
    ...
    private SbaDelivery evaluateParameters(Long version, Long canton, String deliveryCode) {
        SbaDelivery delivery = deliveryRepository.getDeliveryByIdentification(canton, version, deliveryCode);
        if (delivery == null) {
            // No delivery error
            throw new MebUncheckedNotMonitoredException("export.xml.delivery.error.message");
    ...
    private MEBResponseDocument buildXMLDeliveryPlausireportDocument(SbaDelivery delivery, List<SbaPlausiError> plausiErrors, String locale) {
        MEBResponseDocument mebResponseDoc = MEBResponseDocument.Factory.newInstance();

        // Get object types
        List<CodeGroup> objectTypes = codegroupManager.getCodeGroupsByGroupIdAndLanguage(CodegroupUtility.SBA_OBJECTTYPE, locale);
        ...
        for (SbaPlausiError plausiError : plausiErrors) {
            ...
            if (plausiError.getDeliveredQualificationId() != null) {
                validationErrorType.setExNr(plausiError.getDeliveredQualificationId());
            }
        }
    ...
    public Export insertExport(Export export) {
        return exportRepository.insertExport(new SbaExport(export));
    }

    @Override
    public Export updateExport(Export export) {
        SbaExport updatedExport = new SbaExport(export);
        updatedExport.setSbaParameters(exportRepository.getParameters(export.getExportId()));
        return exportRepository.updateExport(updatedExport);
    }

    @Override
    public void deleteExport(Export export) {
        exportRepository.deleteExport(new SbaExport(export));
    }
}
