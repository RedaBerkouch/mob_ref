...
public class SbaBurSchool extends BurSchool {
    @Column
    @ManyToMany(targetEntity = SbaConfigDelivery.class, fetch = FetchType.EAGER, cascade = { CascadeType.PERSIST, CascadeType.MERGE })
    @JoinTable(name = "SBA_SCHOOLS_CONFIGDELIVERIES", joinColumns = @JoinColumn(name = "schoolId"), inverseJoinColumns = @JoinColumn(name = "deliveryId"))
    protected List<SbaConfigDelivery> configDeliveries;
     ...
     * Copy from dto
     */
    public SbaBurSchool(BurSchool dtoBurSchool) {
