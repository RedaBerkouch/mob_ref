...
public class PersonService implements IPersonService {
    ...
    public SbaPersonListResult getPersons(int start, int buffer, WebSortContext sortContext, WebFilterContext filterContext, Long version, Long canton) {
        return webServiceClientFactory.getPersonWebService().getPersons(start, buffer, convertToSortContext(sortContext), convertToFilterContext(filterContext),
                version, canton);
    }
    ...
    public SbaPersonListResult getPersonsOwnedByQualifications(List<Long> qualificationIds, WebSortContext sortContext) {
        return webServiceClientFactory.getPersonWebService().getPersonsOwnedByQualifications(qualificationIds, convertToSortContext(sortContext));
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_RO + "')")
    public SbaPersonResult getPersonById(Long personId) {
        return webServiceClientFactory.getPersonWebService().getPersonById(personId);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_RO + "')")
    public PlausiErrorListResult getPlausiErrorsForPerson(Long personId) {
        return webServiceClientFactory.getPersonWebService().getPlausiErrorsForPerson(personId);
    }
    ...
    public SbaPersonResult updatePerson(SbaPerson person, List<PlausiError> plausiErrors, boolean noPlausi) {
        return webServiceClientFactory.getPersonWebService().updatePerson(person, plausiErrors, noPlausi);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_DL + "')")
    public SbaPersonResult insertPerson(SbaPerson person, boolean noPlausi) {
        return webServiceClientFactory.getPersonWebService().insertPerson(person, noPlausi);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_DL + "')")
    public SbaPersonResult deletePerson(SbaPerson person, boolean noPlausi) {
        return webServiceClientFactory.getPersonWebService().deletePerson(person, noPlausi);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_DL + "')")
    public SbaPersonResult validatePersons(List<Long> personList, boolean undo) {
        return webServiceClientFactory.getPersonWebService().validatePersons(personList, undo);
    }
}
