...
public class QualificationExportService {
    ...
    public byte[] generateCsvExport(List<Long> personIds) {
        SbaQualificationListResult result = qualificationService.getQualificationsOwnedByPersons(
                personIds,
                defaultWebSortContext()
        );
        return generateCsvFromResult(result);
     ...
     */
    private byte[] generateCsvFromResult(SbaQualificationListResult result) {
        CsvExportSheet csv = new CsvExportSheet();

        try {
            csv.add(getColumnHeaders());

            if (result != null && !CollectionUtils.isEmpty(result.getQualifications())) {
                result.getQualifications().stream()
    ...
    private String formatQualificationRow(SbaQualification qualification) {
        return new CsvExportRow()
                ...
                .add(localizationManager.getCodeGroupValueById(CodegroupUtility.EXAM_EDUCATION_TYPE, qualification.getEducationType()))
                .add(localizationManager.getCodeGroupValueById(CodegroupUtility.EXAM_TYPE, qualification.getExamType()))
                .add(qualification.getExamDate())
                .add(qualification.getExamNr())
                .add(localizationManager.getCodeGroupValueById(CodegroupUtility.EXAM_RESULT, qualification.getResult()))
                .add(localizationManager.getCodeGroupValueById(CodegroupUtility.MATURITY_LANGUAGES, qualification.getMaturityLanguages()))
