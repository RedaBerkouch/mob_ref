...
public class QualificationService implements IQualificationService {
    ...
    public SbaQualificationListResult getQualifications(int start, int buffer, WebSortContext sortContext, WebFilterContext filterContext, Long version,
            Long canton) {
        return webServiceClientFactory.getQualificationWebService().getQualifications(start, buffer, convertToSortContext(sortContext),
    ...
    public SbaQualificationListResult getQualificationsOwnedByPersons(List<Long> personIds, WebSortContext sortContext) {
        return webServiceClientFactory.getQualificationWebService().getQualificationsOwnedByPersons(personIds, convertToSortContext(sortContext));
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_RO + "')")
    public SbaQualificationResult getQualificationById(Long qualificationId) {
        return webServiceClientFactory.getQualificationWebService().getQualificationById(qualificationId);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_RO + "')")
    public PlausiErrorListResult getPlausiErrorsForQualification(Long qualificationId) {
        return webServiceClientFactory.getQualificationWebService().getPlausiErrorsForQualification(qualificationId);
    }
    ...
    public SbaQualificationResult updateQualification(SbaQualification qualification, List<PlausiError> plausiErrors, boolean noPlausi) {
        return webServiceClientFactory.getQualificationWebService().updateQualification(qualification, plausiErrors, noPlausi);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_DL + "')")
    public SbaQualificationResult insertQualification(SbaQualification qualification, boolean noPlausi) {
        return webServiceClientFactory.getQualificationWebService().insertQualification(qualification, noPlausi);
    }

    @Override
    @PreAuthorize("hasAuthority('" + SecurityConstants.ROLE_SBA_DL + "')")
    public SbaQualificationResult deleteQualification(SbaQualification qualification, boolean noPlausi) {
        return webServiceClientFactory.getQualificationWebService().deleteQualification(qualification, noPlausi);
    }
}
