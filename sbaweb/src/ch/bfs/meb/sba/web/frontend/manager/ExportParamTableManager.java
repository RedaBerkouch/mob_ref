...
/**
 * This Class represents a ExportParamTableManager for the admin tab and acts as
 * a controller for the ExportParam Table.
 */
@Scope("session")
@Component("exportParamTableManager")
public class ExportParamTableManager extends ParamTableManagerBase {
    @SuppressWarnings("unused")
    private static final Logger LOGGER = LoggerFactory.getLogger(ExportParamTableManager.class);

    public static final String MANAGER_NAME = "exportParam";
    ...
    public IDhtmlxControl getFilterTable() {
        return new IDhtmlxControl() {
            @Override
            public String getControlName() {
                return AdminExportTableManager.CONTROL_NAME;
            }

            @Override
            public String getName() {
                return AdminExportTableManager.MANAGER_NAME;
            }
        };
    }
    ...
    public ParameterListResult getRows(ParameterList params) {
        ...
        if (params.getSelectedRows().size() == 0) {
            ...
            parameters = _parameterService.getParametersForExport(selected);
        }
