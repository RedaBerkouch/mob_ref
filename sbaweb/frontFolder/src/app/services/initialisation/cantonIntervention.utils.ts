...
export function isActionDisabledForCantonIntervention(
	...
	switch (actionType) {
		case "delete":
			return !hasSelected || isDL && type < SBA_CANTONINTERVENTIONTYPE.MANUAL;
		...
		case "show_plausi_report":
			return !hasSelected || type != SBA_CANTONINTERVENTIONTYPE.CREATE_PLAUSIREPORT;

		case "download_file":
			return !hasSelected || type != SBA_CANTONINTERVENTIONTYPE.UPLOAD;
...
export function enrichDefaultValuesForCantonIntervention(columns: DatahandlerColumn[]): DatahandlerColumn[] {
	const defaultValues: Record<string, any> = {
		...
		type: SBA_CANTONINTERVENTIONTYPE.MANUAL
	};
